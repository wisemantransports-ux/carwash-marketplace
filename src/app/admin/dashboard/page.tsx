...
import { Banknote, Users, CreditCard, ShieldCheck, Loader2, RefreshCw } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: "Total Accounts", value: businesses.length.toString(), trend: "Onboarded Users", icon: Users, color: "text-purple-600", bg: "bg-purple-50" },
  ];

  if (!mounted || (loading && !refreshing)) return (
    <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        <Loader2 className="animate-spin text-primary h-10 w-10" />
        <p className="text-sm font-medium animate-pulse">Loading platform analytics...</p>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-lg">Oversight of business verification, billing, and platform growth.</p>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-7">
        <Card className="lg:col-span-4 shadow-xl border-2 overflow-hidden rounded-2xl">
          <CardHeader className="bg-muted/10 border-b flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>The latest partners joining the ecosystem.</CardDescription>
            </div>
             ...
             <Table>
                <TableHeader>
                    <TableRow className="bg-muted/30 border-b-2">
                        <TableHead className="font-bold py-4 pl-6">Partner Name</TableHead>
                ...
                <TableBody>
                    {businesses.slice(0, 5).map(biz => (
                        <TableRow key={biz.id} className="hover:bg-muted/10 transition-colors border-b last:border-0">
                            ...
                            <TableCell>
                                <Badge variant="outline" className="text-[9px] font-black uppercase">{biz.plan || 'Starter'}</Badge>
                            </TableCell>
                            <TableCell>
                                <Badge 
        ...
        <Card className="lg:col-span-3 border-2 shadow-xl rounded-2xl overflow-hidden">
          <CardHeader className="bg-muted/10 border-b">
            ...
            <CardDescription>Manual verifications pending review.</CardDescription>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
             <div className="p-5 rounded-2xl border-2 bg-orange-50/50 border-orange-100">
                ...
                <p className="text-xs text-orange-700 mb-6 font-medium">Review Mobile Money uploads to activate subscription tiers.</p>
             ...
             <div className="p-5 rounded-2xl border-2 bg-blue-50/50 border-blue-100">
                <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-black text-blue-800 uppercase tracking-widest">Verification</span>
                    <Badge className="bg-blue-600 text-white font-black">Active</Badge>
                </div>
                <p className="text-xs text-blue-700 mb-6 font-medium">Inspect ID selfies and CIPA certificates for new operators.</p>
                <Button className="w-full bg-blue-600 hover:bg-blue-700 shadow-lg font-bold" asChild>
                    <Link href="/admin/verification">Registration Center</Link>
                </Button>
             </div>
          </CardContent>
        </Card>
